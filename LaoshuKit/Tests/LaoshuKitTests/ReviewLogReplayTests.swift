...
/// Exercises the D18 replay migration through the real upgrade path: a
/// review log already on the pre-batch schema, opened for the first time
/// after the code that adds batches, same as `LaoshuDatabase`'s own tests
/// for the ladder itself (D21's fixed-clock seam).
