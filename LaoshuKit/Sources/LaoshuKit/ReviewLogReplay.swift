...
/// Registered as a schema migration (see `LaoshuDatabase`) so it inherits
/// that mechanism's guarantees for free: the replay and its completion
/// marker land in one transaction, and `DatabaseMigrator` never re-runs a
...
enum ReviewLogReplay {
    ...
    /// review row, its level — a batch carries exactly one level) and
    /// creates one batch per group, wound forward to where it would stand on
    /// `today`.
